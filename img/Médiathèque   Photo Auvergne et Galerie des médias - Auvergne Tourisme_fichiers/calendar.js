...
var Calendar = new Class({
  ...
  Implements: [Options, Events],
	...
	initialize: function(obj, options) {
		...
		this.options.days.each( function( oItem, iIndex ) {
			this.options.days[iIndex] = MooTools.lang.get('fromPHP', oItem);
		}.bind(this));
		this.options.months.each( function( oItem, iIndex ) {
			this.options.months[iIndex] = MooTools.lang.get('fromPHP', oItem);
		}.bind(this));
		...
		if ( this.options.fix )
		{
			...
			this.calendar = new Element('div', {
				'styles': { left: 0, opacity: 0, position: 'relative', top: 0, zIndex: 99 }
			});
			if ( this.classes.calendar )
				this.calendar.addClass(this.classes.calendar);

			if ( $(this.options.inject) )
				this.calendar.injectInside($(this.options.inject));

			// iex 6 needs a transparent iframe underneath the calendar in order to not allow select elements to render through
			if (window.ie6) {
				this.iframe = new Element('iframe', {
					'styles': { left: 0, position: 'relative', top: 0, zIndex: 98 }
				}).injectInside($(this.options.inject));
				this.iframe.style.filter = 'progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0)';
			}
		}
		else
		{
			var inject = 'datePeriod';
			...
			}).addClass(this.classes.calendar).injectInside(inject);
	...
	display: function(cal) {
		...
		for (var i = 1; i < 43; i++) { // 1 to 42 (6 x 7 or 6 weeks)
			...
			if (valid.contains(day)) { // if it's a valid - clickable - day we add interaction
				td.setProperty('title', this.format(date, 'l j F Y'));
	...
	toggle: function(cal) {
		...
		if ( cal.visible && !this.options.fix ) { // cal.visible
			cal.visible = false;
			...
			if ( this.options.scroll == true)
			{
				...
				var coord = cal.button.getPosition();
				var scroll = window.getScroll();

				if (this.options.inject != '' && !this.options.fix)
				{
					coord = cal.button.getPosition($(this.options.inject));
					scroll = $(this.options.inject).getScroll();
				}
				...
				var x = coord.x + scroll.x + this.options.tweak.x;
				var y = coord.y + scroll.y + this.options.tweak.y;
